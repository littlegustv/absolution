...
load_objects (FILE * fp)
{
   ...
   for (;;)
   {
	 ...
	 pObjIndex->value[0] = weapon_type (fread_word (fp));
	 pObjIndex->value[1] = fread_number(fp);
	 if(pObjIndex->value[1] == SHEATH_SPELL)
		pObjIndex->value[2] = skill_lookup(fread_word (fp));
	 else if(pObjIndex->value[1] == SHEATH_FLAG)
	 ...
	 break;
      ...
      for(i = 0; i < 5; i++)
	 pObjIndex->special[i] = 0;

      if(IS_SET(pObjIndex->wear_flags, ITEM_CHARGED)) {
	 char *special3;

	 pObjIndex->special[0] = fread_number(fp);
	 pObjIndex->special[1] = fread_number(fp);
	 pObjIndex->special[2] = fread_number(fp);
	 special3 = fread_word(fp);
	 if(isdigit(special3[0]))
		pObjIndex->special[3] = atoi(special3);
	 else
	 	pObjIndex->special[3] = skill_lookup(special3);

	 if(pObjIndex->special[3] < 0) {
		log_string("Object loaded with bad charge spell.\n\r");
	 }

	 pObjIndex->special[4] = fread_number(fp);
      }

      // Read in the range for a ranged weapon.
      if(pObjIndex->item_type == ITEM_WEAPON
      && pObjIndex->value[0] == WEAPON_RANGED)
         pObjIndex->extra[2] = fread_number(fp);
	 ...
	 {
		pObjIndex->affected->bitvector |= AFF_DAYLIGHT;
	 }
	 else if (letter == 'N')
         {
                pObjIndex->affected->bitvector |= AFF_DARKLIGHT;
         }
	 else if (letter == 'F')
	 {
	    AFFECT_DATA *paf;
 ...
 ****************************************************************************/
void
convert_object (OBJ_INDEX_DATA * pObjIndex)
{
   int level;
 ...
 ****************************************************************************/
void
convert_mobile (MOB_INDEX_DATA * pMobIndex)
{
   int i;
   ...
   bonus = UMAX (0, (int)(level * (8 + level) * 0.9 - number * type) );
