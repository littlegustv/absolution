...
ban_site (CHAR_DATA * ch, char *argument, bool fPerm)
{
   ...
   if (arg1[0] == '\0')
   {
      if (ban_list == NULL)
      {
	 Cprintf(ch, "No sites banned at this time.\n\r");
	 return;
      }
      buffer = new_buf ();
   ...
   {
      Cprintf(ch, "Acceptable ban types are all, newbies, and permit.\n\r");
      return;
   }
   ...
   if (strlen (name) == 0)
   {
      Cprintf(ch, "You have to ban SOMETHING.\n\r");
      return;
   }
   ...
   for (pban = ban_list; pban != NULL; prev = pban, pban = pban->next)
   {
	 ...
	 {
	    Cprintf(ch, "That ban was set by a higher power.\n\r");
	    return;
	 }
	 else
	 {
...
do_allow (CHAR_DATA * ch, char *argument)
{
   ...
   if (arg[0] == '\0')
   {
      Cprintf(ch, "Remove which site from the ban list?\n\r");
      return;
   }
   ...
   for (curr = ban_list; curr != NULL; prev = curr, curr = curr->next)
   {
	 ...
	 {
	    Cprintf(ch, "You are not powerful enough to lift that ban.\n\r");
	    return;
	 }
	 if (prev == NULL)
	 ...
	 free_ban (curr);
	 Cprintf(ch, "Ban on %s lifted.\n\r", arg);
	 save_bans ();
	 return;
      }
   }

   Cprintf(ch, "Site is not banned.\n\r");
   return;
}
